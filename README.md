...
| `authed-cache-leak` | next | security | gated pages that can render static/shared-cacheable behind a cookie-blind CDN (the VB 8/11 authed-page cache leak): `generateStaticParams` / `revalidate > 0` / `force-static` inside a gated tree (security — the generator wins even over `force-dynamic`), or middleware-gated pages with no dynamic proof (warning). Session-reading trees and no-store-stamping middlewares are recognized as safe shapes; exempt extra public segments via `.fleetcheckrc.json` `{ "authed-cache-leak": { "publicRoutes": [...] } }` |
